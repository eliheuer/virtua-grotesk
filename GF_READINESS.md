...
See `documentation/google-fonts-decision-questions.md` for the short answer
sheet, and `documentation/google-fonts-decisions.md` for the canonical decision
log.
For the fastest maintainer pass, run `make decisions` or
`make decision-readiness-check`, or open
`documentation/google-fonts-decision-answer-sheet.md`; it sorts the same open
...
`documentation/google-fonts-decisions.md`: use the public upstream URL
...
- [x] Keep the legacy `AUTHORS` file as a pointer to the GF-required files.
...
- [x] Add draft `documentation/DESCRIPTION.en_us.html`.
- [x] Add draft `documentation/ARTICLE.en_us.html` for the newer Google Fonts
  Article flow.
- [x] Keep `documentation/DESCRIPTION.en_us.html` public-facing rather than
...
- [x] Add `make kerning-proof-review-check` and
  `documentation/kerning-proof-review.md` so the gftools QA proof review is
  auditable by humans and agents.
- [x] Add `make reference-index-check` and
  `documentation/google-fonts-reference-index.md` so official Google Fonts
...
- [x] Add README specimen image in `documentation/readme-specimen.png`.
...
- `make kerning-proof-check` writes Google Fonts QA HTML proofs to
  `documentation/gftools-qa/` for human spacing and kerning review. This is a
...
The current missing-codepoint checklist is in `documentation/missing-gf-latin-core.md`.
It currently reports 219 missing GF Latin Core codepoints out of 319 required.
The current Arabic missing-codepoint checklist is in `documentation/missing-gf-arabic-core.md`.
...
OpenType layout.
The current glyphset readiness summary is in `documentation/gf-glyphset-readiness.md`.
...
`documentation/google-fonts-language-metadata.md`. It checks the local
...
The current PUA/private-use scope report is in `documentation/pua-scope.md`.
...
The current Arabic shaping smoke test is in `documentation/arabic-shaping-smoke-test.md`.
...
The current Arabic mark-readiness report is in `documentation/arabic-mark-readiness.md`.
...
The current Arabic review packet is in `documentation/arabic-review-packet.md`.
...
handoff review.
The current contour-count checklist is in `documentation/fontspector-contour-count.md`.
It currently reports 117 contour-count findings: 104 empty/no-contour glyphs and 13 unexpected contour-count glyphs.
The current master compatibility report is in `documentation/master-compatibility.md`.
...
The current variable-font metadata report is in `documentation/variable-font-metadata.md`.
...
`documentation/google-fonts-axis-registry-audit.md`. It compares the built
...
The current generated-font metadata report is in `documentation/generated-font-metadata.md`.
...
The current source UFO metadata report is in `documentation/source-ufo-metadata.md`.
...
`documentation/designer-profile-readiness.md`. It checks current author and
...
The current Article draft is in `documentation/ARTICLE.en_us.html`. Its
...
`documentation/google-fonts-template-and-pr-audit.md`. It records how this repo
...
`documentation/recent-google-fonts-packages.md`. It reads selected merged
...
`documentation/google-fonts-add-font-template-audit.md`. It reads the template
...
`documentation/project-template-automation-readiness.md`. It separates
...
`documentation/google-fonts-downstream-package-preview.md`. It records the
expected `ofl/virtuagrotesk` layout and a draft `METADATA.pb` shape for final
packaging review.
The current package source-file audit is in
`documentation/package-source-files-audit.md`. It checks the local files listed
...
`documentation/release-source-readiness.md`. It ties the final Packager source
...
`documentation/release-archive-manifest.md`. It records the local
...
command draft is in `documentation/github-release-draft.md`; the matching
release notes are in `documentation/github-release-notes.md`. Regenerate both
with `make release-draft-check`.
The current upstream structure readiness report is in
`documentation/upstream-structure-readiness.md`. It checks mandatory upstream
...
`documentation/decision-readiness.md`. It checks the open maintainer questions
against `documentation/google-fonts-decisions.md` and the current Add Font
template audit so policy and metadata answers can be applied consistently.
The current priority-sorted answer sheet is in
`documentation/google-fonts-decision-answer-sheet.md`. It is what
`make decisions` prints for a focused maintainer review pass.
The current decision application blocker map is in
`documentation/decision-application-blockers.md`. It shows which remaining
answers block downstream metadata, package dry runs, or final submission only.
The current Article readiness report is in `documentation/article-readiness.md`.
...
`documentation/final-submission-blockers.md`. It aggregates open decisions,
...
`documentation/submission-handoff-readiness.md`. It checks the draft Add Font
...
optional `fontc`.
The current warning report is in `documentation/fontspector-warnings.md`.
...
The current kerning readiness report is in `documentation/kerning-readiness.md`.
...
- Arabic mark positioning: source anchors and built `mark`/`mkmk` GPOS features
  are not complete yet; track this in `documentation/arabic-mark-readiness.md`.
...
- Article draft: added `documentation/ARTICLE.en_us.html` for the newer Google
  Fonts Article flow.
- Downstream preview: added `documentation/google-fonts-downstream-package-preview.md`
...
- README image: rendered `documentation/readme-specimen.png` with DesignBot from the current build.
...
- Generated metadata: added `documentation/generated-font-metadata.md` to audit
  built font names, versions, metrics, license strings, and script tags.
- Source metadata: added `documentation/source-ufo-metadata.md` to audit active
  UFO names, versions, metrics, license strings, embedding state, and glyph
  counts.
- Release checklist: drafted `documentation/google-fonts-release-checklist.md`
  so the final upstream tag, commit, version, and source URL are recorded before
  downstream packaging.
- Fontspector warnings: generated `documentation/fontspector-warnings.md` for review alongside the drawing reports.
