...
`documentation/image-license.txt`. Copy it alongside `ARTICLE.en_us.html` if
...
- Confirm `documentation/package-source-files-audit.md` agrees with the final
...
- Confirm no custom `sample_text` block is present unless the need is recorded
  in `documentation/google-fonts-decisions.md`.
