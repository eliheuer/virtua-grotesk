...
- Used in `documentation/google-fonts/google-fonts-submission-handoff.md`.
...
- The final `METADATA.pb` must either point at a branch/tag where the listed
  ...
  that contains them.
- `documentation/google-fonts/package-source-files-audit.md` shows the current served
...
- `documentation/google-fonts/pua-scope.md` records the current generated PUA inventory and
...
- Keep `documentation/google-fonts/DESCRIPTION.en_us.html` as a shorter legacy fallback only
  if Google Fonts review asks for it.

Current state:

- `documentation/google-fonts/ARTICLE.en_us.html` exists as the upstream Article draft.
- `documentation/google-fonts/DESCRIPTION.en_us.html` exists as a shorter legacy description.
