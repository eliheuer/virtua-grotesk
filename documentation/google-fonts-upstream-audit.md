...
- Legacy `AUTHORS` is retained only as a pointer to the Google Fonts-required
  `AUTHORS.txt` and `CONTRIBUTORS.txt` files.
...
- `make preflight` builds once, regenerates `proof.pdf`, regenerates reports,
...
- `make handoff` uses the same proof-before-report path for final handoff
  review.
- `documentation/google-fonts-template-and-pr-audit.md` compares this repo to
...
- `documentation/google-fonts-downstream-package-preview.md` records the
...
- `documentation/final-submission-blockers.md` aggregates the current decision,
...
Resolve these in `documentation/google-fonts-decisions.md` before final
...
- Whether the PUA/private icon block ships, using
  `documentation/pua-scope.md` as the current inventory.
- Vendor ID.
