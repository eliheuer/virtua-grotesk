# Arabic Manual Review Batches

This generated report turns the remaining Arabic visual-review and contour-decision queues into a short hand-cleanup order. It does not make design decisions; it gives the fastest sequence for recording them.

Authoritative logs:

- `documentation/glyph-review/arabic-visual-review-log.md`
- `documentation/glyph-review/contour-cleanup/contour-cleanup-decision-log.md`
- `documentation/glyph-review/contour-cleanup/contour-cleanup-source-edit-runlist.md`
- `documentation/glyph-review/contour-cleanup/contour-cleanup-first-edit-batch.md`
- `documentation/glyph-review/arabic-manual-review-dashboard.html`
- `documentation/glyph-review/arabic-next-review-batch.html`
- `documentation/glyph-review/arabic-next-review-snapshots.md`
- `documentation/glyph-review/arabic-first-review-zoom-snapshots.md`
- `documentation/glyph-review/arabic-snapshot-integrity.md`
- `documentation/glyph-review/arabic-full-queue-ai-sweep.md`
- `documentation/google-fonts/gftools-qa/Proof/`

Snapshot evidence:

- Snapshot evidence ready for hand review: yes
- Readable PNG files: 33
- Nonblank PNG files: 33
- Pending/fix-needed rows without snapshot: 0
- Focused first-batch zoom crops: `documentation/glyph-review/arabic-first-review-zoom-snapshots.md`

## Next Unresolved Batch

Start with **2. Structure And Wrong-Glyph Sweep**.

- Why: Catch missing, blank, clipped, duplicated, malformed, or wrong-codepoint glyphs before judging spacing.
- Open decisions: 9
- Visual rows: 5 (pending: 5)
- Contour rows: 4 (pending: 4)
- Decision rule: Mark contour rows `fix-now` for source edits, or `accepted` only after comparing source and rendered proof.

Evidence to open:

- `documentation/glyph-review/arabic-structure-sweep.html`
- `documentation/glyph-review/arabic-structure-triage.md`
- `documentation/google-fonts/gftools-qa/Proof/Regular-diffbrowsers_glyphs.html`
- `documentation/glyph-review/arabic-manual-review-dashboard.html`
- `documentation/google-fonts/gftools-qa/Proof/Medium-diffbrowsers_glyphs.html`
- `documentation/google-fonts/gftools-qa/Proof/SemiBold-diffbrowsers_glyphs.html`
- `documentation/google-fonts/gftools-qa/Proof/Bold-diffbrowsers_glyphs.html`
- `documentation/glyph-review/contour-cleanup/contour-cleanup-decision-log.md`
- `documentation/glyph-review/arabic-cleanup-drawing-briefs.md`
- `documentation/glyph-review/arabic-visual-risk-proof.html`

Snapshot aids:

- `proof-regular-glyphs` Regular glyphs: `documentation/glyph-review/review-snapshots/proof-regular-glyphs.png` from `documentation/google-fonts/gftools-qa/Proof/Regular-diffbrowsers_glyphs.html`
- `proof-regular-glyphs` Regular Arabic glyph rows focused 2x crop: `documentation/glyph-review/review-snapshots/proof-regular-glyphs-arabic-zoom.png` from `documentation/glyph-review/review-snapshots/proof-regular-glyphs.png`
- `proof-medium-glyphs` Medium glyphs: `documentation/glyph-review/review-snapshots/proof-medium-glyphs.png` from `documentation/google-fonts/gftools-qa/Proof/Medium-diffbrowsers_glyphs.html`
- `proof-medium-glyphs` Medium Arabic glyph rows focused 2x crop: `documentation/glyph-review/review-snapshots/proof-medium-glyphs-arabic-zoom.png` from `documentation/glyph-review/review-snapshots/proof-medium-glyphs.png`
- `proof-semibold-glyphs` SemiBold glyphs: `documentation/glyph-review/review-snapshots/proof-semibold-glyphs.png` from `documentation/google-fonts/gftools-qa/Proof/SemiBold-diffbrowsers_glyphs.html`
- `proof-semibold-glyphs` SemiBold Arabic glyph rows focused 2x crop: `documentation/glyph-review/review-snapshots/proof-semibold-glyphs-arabic-zoom.png` from `documentation/glyph-review/review-snapshots/proof-semibold-glyphs.png`
- `proof-bold-glyphs` Bold glyphs: `documentation/glyph-review/review-snapshots/proof-bold-glyphs.png` from `documentation/google-fonts/gftools-qa/Proof/Bold-diffbrowsers_glyphs.html`
- `proof-bold-glyphs` Bold Arabic glyph rows focused 2x crop: `documentation/glyph-review/review-snapshots/proof-bold-glyphs-arabic-zoom.png` from `documentation/glyph-review/review-snapshots/proof-bold-glyphs.png`
- `class-letter-structures` Arabic structure sweep: `documentation/glyph-review/review-snapshots/class-letter-structures.png` from `documentation/glyph-review/arabic-structure-sweep.html`
- `class-letter-structures` Arabic visual risk proof: `documentation/glyph-review/review-snapshots/class-letter-structures-2.png` from `documentation/glyph-review/arabic-visual-risk-proof.html`

First visual-review command pattern:

```bash
make arabic-visual-review-update REVIEW_KEY=proof-regular-glyphs REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"
```

First contour-decision command patterns:

```bash
make contour-decision-update GLYPH=dad-ar.init STATUS=fix-now DECISION="needs source edit" REVIEWED="Name YYYY-MM-DD"
make contour-decision-update GLYPH=dad-ar.init STATUS=accepted DECISION="reviewed style divergence" REVIEWED="Name YYYY-MM-DD"
```

## Batch Queue

### 1. Open The Fast Dashboard

Start with one screen that shows embedded Arabic samples, visual-risk rows, contour previews, and proof links.

- Visual rows: 0 (none)
- Contour rows: 0 (none)
- Decision rule: Use this only to orient the review; record pass/fix decisions in the logs below.

Commands:

```bash
make arabic-manual-review-dashboard
```
```bash
open documentation/glyph-review/arabic-manual-review-dashboard.html
```
```bash
open documentation/glyph-review/arabic-next-review-batch.html
```

### 2. Structure And Wrong-Glyph Sweep

Catch missing, blank, clipped, duplicated, malformed, or wrong-codepoint glyphs before judging spacing.

- Visual rows: 5 (pending: 5)
- Contour rows: 4 (pending: 4)
- Decision rule: Mark contour rows `fix-now` for source edits, or `accepted` only after comparing source and rendered proof.

Evidence to open:

- `documentation/glyph-review/arabic-structure-sweep.html`
- `documentation/glyph-review/arabic-structure-triage.md`
- `documentation/google-fonts/gftools-qa/Proof/Regular-diffbrowsers_glyphs.html`
- `documentation/glyph-review/arabic-manual-review-dashboard.html`
- `documentation/google-fonts/gftools-qa/Proof/Medium-diffbrowsers_glyphs.html`
- `documentation/google-fonts/gftools-qa/Proof/SemiBold-diffbrowsers_glyphs.html`
- `documentation/google-fonts/gftools-qa/Proof/Bold-diffbrowsers_glyphs.html`
- `documentation/glyph-review/contour-cleanup/contour-cleanup-decision-log.md`
- `documentation/glyph-review/arabic-cleanup-drawing-briefs.md`
- `documentation/glyph-review/arabic-visual-risk-proof.html`

Snapshot aids:

- `proof-regular-glyphs` Regular glyphs: `documentation/glyph-review/review-snapshots/proof-regular-glyphs.png` from `documentation/google-fonts/gftools-qa/Proof/Regular-diffbrowsers_glyphs.html`
- `proof-regular-glyphs` Regular Arabic glyph rows focused 2x crop: `documentation/glyph-review/review-snapshots/proof-regular-glyphs-arabic-zoom.png` from `documentation/glyph-review/review-snapshots/proof-regular-glyphs.png`
- `proof-medium-glyphs` Medium glyphs: `documentation/glyph-review/review-snapshots/proof-medium-glyphs.png` from `documentation/google-fonts/gftools-qa/Proof/Medium-diffbrowsers_glyphs.html`
- `proof-medium-glyphs` Medium Arabic glyph rows focused 2x crop: `documentation/glyph-review/review-snapshots/proof-medium-glyphs-arabic-zoom.png` from `documentation/glyph-review/review-snapshots/proof-medium-glyphs.png`
- `proof-semibold-glyphs` SemiBold glyphs: `documentation/glyph-review/review-snapshots/proof-semibold-glyphs.png` from `documentation/google-fonts/gftools-qa/Proof/SemiBold-diffbrowsers_glyphs.html`
- `proof-semibold-glyphs` SemiBold Arabic glyph rows focused 2x crop: `documentation/glyph-review/review-snapshots/proof-semibold-glyphs-arabic-zoom.png` from `documentation/glyph-review/review-snapshots/proof-semibold-glyphs.png`
- `proof-bold-glyphs` Bold glyphs: `documentation/glyph-review/review-snapshots/proof-bold-glyphs.png` from `documentation/google-fonts/gftools-qa/Proof/Bold-diffbrowsers_glyphs.html`
- `proof-bold-glyphs` Bold Arabic glyph rows focused 2x crop: `documentation/glyph-review/review-snapshots/proof-bold-glyphs-arabic-zoom.png` from `documentation/glyph-review/review-snapshots/proof-bold-glyphs.png`
- `class-letter-structures` Arabic structure sweep: `documentation/glyph-review/review-snapshots/class-letter-structures.png` from `documentation/glyph-review/arabic-structure-sweep.html`
- `class-letter-structures` Arabic visual risk proof: `documentation/glyph-review/review-snapshots/class-letter-structures-2.png` from `documentation/glyph-review/arabic-visual-risk-proof.html`

Visual review rows:

| Key | Area | Item | Machine precheck | Review cue | AI observation | Human follow-up | Status | Record command |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `proof-regular-glyphs` | GF proof | Regular glyphs | Structure triage mechanical blockers: 0; structure review prompts: 66 | Glyphs proof: missing, clipped, blank, malformed, duplicated, or wrong-codepoint Arabic glyphs | Full snapshot and focused 2x Arabic-row crop are nonblank. Use the crop to screen structure faster, then open the full glyph proof for missing, blank, clipped, duplicated, malformed, or wrong-codepoint Arabic glyphs. | Open the matching gftools glyph proof at zoom. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-regular-glyphs REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-medium-glyphs` | GF proof | Medium glyphs | Structure triage mechanical blockers: 0; structure review prompts: 66 | Glyphs proof: missing, clipped, blank, malformed, duplicated, or wrong-codepoint Arabic glyphs | Full snapshot and focused 2x Arabic-row crop are nonblank. Use the crop to screen structure faster, then open the full glyph proof for missing, blank, clipped, duplicated, malformed, or wrong-codepoint Arabic glyphs. | Open the matching gftools glyph proof at zoom. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-medium-glyphs REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-semibold-glyphs` | GF proof | SemiBold glyphs | Structure triage mechanical blockers: 0; structure review prompts: 66 | Glyphs proof: missing, clipped, blank, malformed, duplicated, or wrong-codepoint Arabic glyphs | Full snapshot and focused 2x Arabic-row crop are nonblank. Use the crop to screen structure faster, then open the full glyph proof for missing, blank, clipped, duplicated, malformed, or wrong-codepoint Arabic glyphs. | Open the matching gftools glyph proof at zoom. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-semibold-glyphs REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-bold-glyphs` | GF proof | Bold glyphs | Structure triage mechanical blockers: 0; structure review prompts: 66 | Glyphs proof: missing, clipped, blank, malformed, duplicated, or wrong-codepoint Arabic glyphs | Full snapshot and focused 2x Arabic-row crop are nonblank. Use the crop to screen structure faster, then open the full glyph proof for missing, blank, clipped, duplicated, malformed, or wrong-codepoint Arabic glyphs. | Open the matching gftools glyph proof at zoom. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-bold-glyphs REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `class-letter-structures` | Glyph class | letter-structures | Contour decisions pending: 4; fix-now: 0 | sad, dad, tah, zah, meem, heh, wawHamzaabove, lam-alef forms; review sidebearing-risk glyphs in the focused proof | Structure/risk snapshots show the expected high-risk overhang families. The overhangs need shaped-context judgment, not automatic sidebearing edits. | Open `documentation/glyph-review/arabic-manual-review-dashboard.html` and linked reports. | pending | `make arabic-visual-review-update REVIEW_KEY=class-letter-structures REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |

Contour decision rows:

| Source glyph | Fontspector glyph | Category | Status | Fix command | Accept command |
| --- | --- | --- | --- | --- | --- |
| `dad-ar.init` | `uni0636.init` | source outline review | pending | `make contour-decision-update GLYPH=dad-ar.init STATUS=fix-now DECISION="needs source edit" REVIEWED="Name YYYY-MM-DD"` | `make contour-decision-update GLYPH=dad-ar.init STATUS=accepted DECISION="reviewed style divergence" REVIEWED="Name YYYY-MM-DD"` |
| `hah-ar.fina` | `uni062D.fina` | source outline review | pending | `make contour-decision-update GLYPH=hah-ar.fina STATUS=fix-now DECISION="needs source edit" REVIEWED="Name YYYY-MM-DD"` | `make contour-decision-update GLYPH=hah-ar.fina STATUS=accepted DECISION="reviewed style divergence" REVIEWED="Name YYYY-MM-DD"` |
| `jeem-ar.fina` | `uni062C.fina` | source outline review | pending | `make contour-decision-update GLYPH=jeem-ar.fina STATUS=fix-now DECISION="needs source edit" REVIEWED="Name YYYY-MM-DD"` | `make contour-decision-update GLYPH=jeem-ar.fina STATUS=accepted DECISION="reviewed style divergence" REVIEWED="Name YYYY-MM-DD"` |
| `sad-ar.init` | `uni0635.init` | source outline review | pending | `make contour-decision-update GLYPH=sad-ar.init STATUS=fix-now DECISION="needs source edit" REVIEWED="Name YYYY-MM-DD"` | `make contour-decision-update GLYPH=sad-ar.init STATUS=accepted DECISION="reviewed style divergence" REVIEWED="Name YYYY-MM-DD"` |

### 3. Marks, Dotted Circle, And Stacking

Arabic Core is mechanically present, but marks still need visual attachment and stacking review.

- Visual rows: 8 (pending: 8)
- Contour rows: 0 (none)
- Decision rule: Review real-base attachment and dotted-circle behavior before accepting or editing mark composites.

Evidence to open:

- `documentation/glyph-review/arabic-mark-review-proof.html`
- `documentation/glyph-review/arabic-mark-triage.md`
- `documentation/glyph-review/arabic-mark-readiness.md`
- `documentation/glyph-review/arabic-manual-review-dashboard.html`
- `documentation/google-fonts/gftools-qa/Proof`
- `documentation/glyph-review/contour-cleanup/contour-cleanup-decision-log.md`
- `documentation/glyph-review/arabic-cleanup-drawing-briefs.md`

Snapshot aids:

- `mark-base+fatha` Arabic mark proof: `documentation/glyph-review/review-snapshots/mark-base+fatha.png` from `documentation/glyph-review/arabic-mark-review-proof.html`
- `mark-base+damma` Arabic mark proof: `documentation/glyph-review/review-snapshots/mark-base+damma.png` from `documentation/glyph-review/arabic-mark-review-proof.html`
- `mark-base+kasra` Arabic mark proof: `documentation/glyph-review/review-snapshots/mark-base+kasra.png` from `documentation/glyph-review/arabic-mark-review-proof.html`
- `mark-shadda+sukun` Arabic mark proof: `documentation/glyph-review/review-snapshots/mark-shadda+sukun.png` from `documentation/glyph-review/arabic-mark-review-proof.html`
- `mark-tanween` Arabic mark proof: `documentation/glyph-review/review-snapshots/mark-tanween.png` from `documentation/glyph-review/arabic-mark-review-proof.html`
- `mark-hamza-above-below` Arabic mark proof: `documentation/glyph-review/review-snapshots/mark-hamza-above-below.png` from `documentation/glyph-review/arabic-mark-review-proof.html`
- `mark-dotted-circle` Arabic mark proof: `documentation/glyph-review/review-snapshots/mark-dotted-circle.png` from `documentation/glyph-review/arabic-mark-review-proof.html`
- `class-mark-combinations` Arabic mark proof: `documentation/glyph-review/review-snapshots/class-mark-combinations.png` from `documentation/glyph-review/arabic-mark-review-proof.html`

Visual review rows:

| Key | Area | Item | Machine precheck | Review cue | AI observation | Human follow-up | Status | Record command |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `mark-base+fatha` | Mark attachment | base+fatha | Mark triage mechanical blockers: 0; mark no-offset prompts: 10 | top mark position clears the base and matches style | Section-targeted mark snapshot shows fatha samples across all five generated fonts. Human review still needs top-mark clearance, centering, and angle checks. | Open `documentation/glyph-review/arabic-mark-review-proof.html`. | pending | `make arabic-visual-review-update REVIEW_KEY=mark-base+fatha REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `mark-base+damma` | Mark attachment | base+damma | Mark triage mechanical blockers: 0; mark no-offset prompts: 10 | damma position and scale are readable across weights | Section-targeted mark snapshot shows damma samples across all five generated fonts. Human review still needs damma scale and Bold readability checks. | Open `documentation/glyph-review/arabic-mark-review-proof.html`. | pending | `make arabic-visual-review-update REVIEW_KEY=mark-base+damma REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `mark-base+kasra` | Mark attachment | base+kasra | Mark triage mechanical blockers: 0; mark no-offset prompts: 10 | bottom mark position clears descenders and sidebearings | Section-targeted mark snapshot shows kasra samples across all five generated fonts. Human review still needs bottom-mark clearance and sidebearing checks. | Open `documentation/glyph-review/arabic-mark-review-proof.html`. | pending | `make arabic-visual-review-update REVIEW_KEY=mark-base+kasra REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `mark-shadda+sukun` | Mark attachment | shadda+sukun | Mark triage mechanical blockers: 0; mark no-offset prompts: 10 | stacked top marks remain clear and centered | Section-targeted mark snapshot shows shadda, sukun, and stacked composites across all five generated fonts. Prioritize the no-offset prompts for `بُّ` and `بَّ`. | Open `documentation/glyph-review/arabic-mark-review-proof.html`. | pending | `make arabic-visual-review-update REVIEW_KEY=mark-shadda+sukun REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `mark-tanween` | Mark attachment | tanween | Mark triage mechanical blockers: 0; mark no-offset prompts: 10 | tanween combinations remain clear and aligned | Section-targeted mark snapshot shows tanween samples across all five generated fonts. Human review still needs twin-mark clarity and alignment checks. | Open `documentation/glyph-review/arabic-mark-review-proof.html`. | pending | `make arabic-visual-review-update REVIEW_KEY=mark-tanween REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `mark-hamza-above-below` | Mark attachment | hamza-above-below | Mark triage mechanical blockers: 0; mark no-offset prompts: 10 | hamza combinations attach cleanly above and below | Section-targeted mark snapshot shows hamza-above and hamza-below samples across all five generated fonts. Human review still needs above/below clearance checks. | Open `documentation/glyph-review/arabic-mark-review-proof.html`. | pending | `make arabic-visual-review-update REVIEW_KEY=mark-hamza-above-below REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `mark-dotted-circle` | Mark attachment | dotted-circle | Mark triage mechanical blockers: 0; mark no-offset prompts: 10 | dotted circle with top and bottom marks is readable | Section-targeted mark snapshot shows dotted circle with top, bottom, and tanween marks across all five generated fonts. Human review still needs dotted-circle readability checks. | Open `documentation/glyph-review/arabic-mark-review-proof.html`. | pending | `make arabic-visual-review-update REVIEW_KEY=mark-dotted-circle REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `class-mark-combinations` | Glyph class | mark-combinations | Mark triage mechanical blockers: 0; mark no-offset prompts: 10 | shadda, hamza, tanween, sukun, and kasra composites | Use the shared mark proof to compare composite mark scale and stacking; mechanical mark setup is present, but visual approval is still pending. | Open `documentation/glyph-review/arabic-mark-review-proof.html`. | pending | `make arabic-visual-review-update REVIEW_KEY=class-mark-combinations REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |

### 4. Dot-Stack Helpers And Urdu/Persian Texture

Three-dot and six-dot helpers are likely to need Bold/variable separation checks.

- Visual rows: 1 (pending: 1)
- Contour rows: 0 (none)
- Decision rule: Fix only if dots merge, collide, or break the intended geometric texture.

Evidence to open:

- `documentation/glyph-review/contour-cleanup/contour-cleanup-decision-log.md`
- `documentation/glyph-review/arabic-cleanup-drawing-briefs.md`
- `documentation/glyph-review/arabic-manual-review-dashboard.html`

Snapshot aids:

- `class-dot-stack-helpers` Arabic manual dashboard: `documentation/glyph-review/review-snapshots/class-dot-stack-helpers.png` from `documentation/glyph-review/arabic-manual-review-dashboard.html`

Visual review rows:

| Key | Area | Item | Machine precheck | Review cue | AI observation | Human follow-up | Status | Record command |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `class-dot-stack-helpers` | Glyph class | dot-stack-helpers | Contour decisions pending: 4; fix-now: 0 | three-dot and six-dot Persian/Urdu helpers | Section-targeted dashboard snapshot shows Persian/Urdu dotted letters across all generated fonts. Use it to inspect dot separation, especially in Bold and in the variable font. | Open `documentation/glyph-review/arabic-manual-review-dashboard.html` and linked reports. | pending | `make arabic-visual-review-update REVIEW_KEY=class-dot-stack-helpers REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |

### 5. RTL Text, Punctuation, Numerals, And Spacing

Once glyph structures look sane, review the typography in proof text, proofer, and waterfall views.

- Visual rows: 18 (pending: 18)
- Contour rows: 0 (none)
- Decision rule: Use `fix-needed` for visual log rows when spacing or rhythm needs drawing work; use `pass` only after checking all weights.

Evidence to open:

- `documentation/google-fonts/gftools-qa/Proof/Regular-diffbrowsers_text.html`
- `documentation/glyph-review/arabic-manual-review-dashboard.html`
- `documentation/google-fonts/gftools-qa/Proof/Regular-diffbrowsers_proofer.html`
- `documentation/google-fonts/gftools-qa/Proof/Regular-diffbrowsers_waterfall.html`
- `documentation/google-fonts/gftools-qa/Proof/Medium-diffbrowsers_text.html`
- `documentation/google-fonts/gftools-qa/Proof/Medium-diffbrowsers_proofer.html`
- `documentation/google-fonts/gftools-qa/Proof/Medium-diffbrowsers_waterfall.html`
- `documentation/google-fonts/gftools-qa/Proof/SemiBold-diffbrowsers_text.html`
- `documentation/google-fonts/gftools-qa/Proof/SemiBold-diffbrowsers_proofer.html`
- `documentation/google-fonts/gftools-qa/Proof/SemiBold-diffbrowsers_waterfall.html`
- `documentation/google-fonts/gftools-qa/Proof/Bold-diffbrowsers_text.html`
- `documentation/google-fonts/gftools-qa/Proof/Bold-diffbrowsers_proofer.html`
- `documentation/google-fonts/gftools-qa/Proof/Bold-diffbrowsers_waterfall.html`
- `documentation/glyph-review/arabic-shaping-smoke-test.md`
- `documentation/glyph-review/contour-cleanup/contour-cleanup-decision-log.md`
- `documentation/glyph-review/arabic-cleanup-drawing-briefs.md`

Snapshot aids:

- `proof-regular-text` Regular text: `documentation/glyph-review/review-snapshots/proof-regular-text.png` from `documentation/google-fonts/gftools-qa/Proof/Regular-diffbrowsers_text.html`
- `proof-regular-proofer` Regular proofer: `documentation/glyph-review/review-snapshots/proof-regular-proofer.png` from `documentation/google-fonts/gftools-qa/Proof/Regular-diffbrowsers_proofer.html`
- `proof-regular-waterfall` Regular waterfall: `documentation/glyph-review/review-snapshots/proof-regular-waterfall.png` from `documentation/google-fonts/gftools-qa/Proof/Regular-diffbrowsers_waterfall.html`
- `proof-medium-text` Medium text: `documentation/glyph-review/review-snapshots/proof-medium-text.png` from `documentation/google-fonts/gftools-qa/Proof/Medium-diffbrowsers_text.html`
- `proof-medium-proofer` Medium proofer: `documentation/glyph-review/review-snapshots/proof-medium-proofer.png` from `documentation/google-fonts/gftools-qa/Proof/Medium-diffbrowsers_proofer.html`
- `proof-medium-waterfall` Medium waterfall: `documentation/glyph-review/review-snapshots/proof-medium-waterfall.png` from `documentation/google-fonts/gftools-qa/Proof/Medium-diffbrowsers_waterfall.html`
- `proof-semibold-text` SemiBold text: `documentation/glyph-review/review-snapshots/proof-semibold-text.png` from `documentation/google-fonts/gftools-qa/Proof/SemiBold-diffbrowsers_text.html`
- `proof-semibold-proofer` SemiBold proofer: `documentation/glyph-review/review-snapshots/proof-semibold-proofer.png` from `documentation/google-fonts/gftools-qa/Proof/SemiBold-diffbrowsers_proofer.html`
- `proof-semibold-waterfall` SemiBold waterfall: `documentation/glyph-review/review-snapshots/proof-semibold-waterfall.png` from `documentation/google-fonts/gftools-qa/Proof/SemiBold-diffbrowsers_waterfall.html`
- `proof-bold-text` Bold text: `documentation/glyph-review/review-snapshots/proof-bold-text.png` from `documentation/google-fonts/gftools-qa/Proof/Bold-diffbrowsers_text.html`
- `proof-bold-proofer` Bold proofer: `documentation/glyph-review/review-snapshots/proof-bold-proofer.png` from `documentation/google-fonts/gftools-qa/Proof/Bold-diffbrowsers_proofer.html`
- `proof-bold-waterfall` Bold waterfall: `documentation/glyph-review/review-snapshots/proof-bold-waterfall.png` from `documentation/google-fonts/gftools-qa/Proof/Bold-diffbrowsers_waterfall.html`
- `smoke-salaam` Arabic manual dashboard: `documentation/glyph-review/review-snapshots/smoke-salaam.png` from `documentation/glyph-review/arabic-manual-review-dashboard.html`
- `smoke-arabic` Arabic manual dashboard: `documentation/glyph-review/review-snapshots/smoke-arabic.png` from `documentation/glyph-review/arabic-manual-review-dashboard.html`
- `smoke-bismillah` Arabic manual dashboard: `documentation/glyph-review/review-snapshots/smoke-bismillah.png` from `documentation/glyph-review/arabic-manual-review-dashboard.html`
- `smoke-lam-alef` Arabic manual dashboard: `documentation/glyph-review/review-snapshots/smoke-lam-alef.png` from `documentation/glyph-review/arabic-manual-review-dashboard.html`
- `class-arabic-farsi-numerals` Arabic manual dashboard: `documentation/glyph-review/review-snapshots/class-arabic-farsi-numerals.png` from `documentation/glyph-review/arabic-manual-review-dashboard.html`
- `class-arabic-punctuation` Arabic manual dashboard: `documentation/glyph-review/review-snapshots/class-arabic-punctuation.png` from `documentation/glyph-review/arabic-manual-review-dashboard.html`

Visual review rows:

| Key | Area | Item | Machine precheck | Review cue | AI observation | Human follow-up | Status | Record command |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `proof-regular-text` | GF proof | Regular text | Matching proof files present: 1 | Text proof: RTL texture, fallback, mark collisions, and unexpected spacing influence | Text snapshot renders mixed Latin/Arabic text without obvious blank-page failure. Inspect the full text proof for RTL texture, fallback, and mark collisions. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-regular-text REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-regular-proofer` | GF proof | Regular proofer | Matching proof files present: 1 | Proofer: sidebearing rhythm, punctuation spacing, numeral rhythm, and weight-specific spacing | Proofer snapshot currently reflects GF_Latin_Core content and shows many box/tofu cells from missing Latin Core coverage. Treat it as a Latin/Core coverage blocker context, not Arabic drawing proof by itself. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-regular-proofer REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-regular-waterfall` | GF proof | Regular waterfall | Matching proof files present: 1 | Waterfall: small-size behavior, interpolation, and mark clarity | Waterfall snapshot is available for size/interpolation checks. Open the HTML at multiple sizes before judging small-size Arabic mark clarity. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-regular-waterfall REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-medium-text` | GF proof | Medium text | Matching proof files present: 1 | Text proof: RTL texture, fallback, mark collisions, and unexpected spacing influence | Text snapshot renders mixed Latin/Arabic text without obvious blank-page failure. Inspect the full text proof for RTL texture, fallback, and mark collisions. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-medium-text REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-medium-proofer` | GF proof | Medium proofer | Matching proof files present: 1 | Proofer: sidebearing rhythm, punctuation spacing, numeral rhythm, and weight-specific spacing | Proofer snapshot currently reflects GF_Latin_Core content and shows many box/tofu cells from missing Latin Core coverage. Treat it as a Latin/Core coverage blocker context, not Arabic drawing proof by itself. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-medium-proofer REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-medium-waterfall` | GF proof | Medium waterfall | Matching proof files present: 1 | Waterfall: small-size behavior, interpolation, and mark clarity | Waterfall snapshot is available for size/interpolation checks. Open the HTML at multiple sizes before judging small-size Arabic mark clarity. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-medium-waterfall REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-semibold-text` | GF proof | SemiBold text | Matching proof files present: 1 | Text proof: RTL texture, fallback, mark collisions, and unexpected spacing influence | Text snapshot renders mixed Latin/Arabic text without obvious blank-page failure. Inspect the full text proof for RTL texture, fallback, and mark collisions. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-semibold-text REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-semibold-proofer` | GF proof | SemiBold proofer | Matching proof files present: 1 | Proofer: sidebearing rhythm, punctuation spacing, numeral rhythm, and weight-specific spacing | Proofer snapshot currently reflects GF_Latin_Core content and shows many box/tofu cells from missing Latin Core coverage. Treat it as a Latin/Core coverage blocker context, not Arabic drawing proof by itself. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-semibold-proofer REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-semibold-waterfall` | GF proof | SemiBold waterfall | Matching proof files present: 1 | Waterfall: small-size behavior, interpolation, and mark clarity | Waterfall snapshot is available for size/interpolation checks. Open the HTML at multiple sizes before judging small-size Arabic mark clarity. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-semibold-waterfall REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-bold-text` | GF proof | Bold text | Matching proof files present: 1 | Text proof: RTL texture, fallback, mark collisions, and unexpected spacing influence | Text snapshot renders mixed Latin/Arabic text without obvious blank-page failure. Inspect the full text proof for RTL texture, fallback, and mark collisions. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-bold-text REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-bold-proofer` | GF proof | Bold proofer | Matching proof files present: 1 | Proofer: sidebearing rhythm, punctuation spacing, numeral rhythm, and weight-specific spacing | Proofer snapshot currently reflects GF_Latin_Core content and shows many box/tofu cells from missing Latin Core coverage. Treat it as a Latin/Core coverage blocker context, not Arabic drawing proof by itself. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-bold-proofer REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `proof-bold-waterfall` | GF proof | Bold waterfall | Matching proof files present: 1 | Waterfall: small-size behavior, interpolation, and mark clarity | Waterfall snapshot is available for size/interpolation checks. Open the HTML at multiple sizes before judging small-size Arabic mark clarity. | Open the matching gftools proof HTML. | pending | `make arabic-visual-review-update REVIEW_KEY=proof-bold-waterfall REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `smoke-salaam` | Smoke string | salaam | Shaping smoke mechanical pass: yes | contextual forms and lam-alef behavior look intentional | Dashboard smoke strings are visible across variable/static weights. Use the dashboard and shaping report to judge join rhythm and style fit. | Open `documentation/glyph-review/arabic-manual-review-dashboard.html` and linked reports. | pending | `make arabic-visual-review-update REVIEW_KEY=smoke-salaam REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `smoke-arabic` | Smoke string | arabic | Shaping smoke mechanical pass: yes | initial, medial, and final joins are shaped and spaced coherently | Dashboard smoke strings are visible across variable/static weights. Use the dashboard and shaping report to judge join rhythm and style fit. | Open `documentation/glyph-review/arabic-manual-review-dashboard.html` and linked reports. | pending | `make arabic-visual-review-update REVIEW_KEY=smoke-arabic REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `smoke-bismillah` | Smoke string | bismillah | Shaping smoke mechanical pass: yes | word spacing, medial joins, heh, and meem forms hold together | Dashboard smoke strings are visible across variable/static weights. Use the dashboard and shaping report to judge join rhythm and style fit. | Open `documentation/glyph-review/arabic-manual-review-dashboard.html` and linked reports. | pending | `make arabic-visual-review-update REVIEW_KEY=smoke-bismillah REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `smoke-lam-alef` | Smoke string | lam-alef | Shaping smoke mechanical pass: yes | lam-alef ligature is present and weight-compatible | Dashboard smoke strings are visible across variable/static weights. Use the dashboard and shaping report to judge join rhythm and style fit. | Open `documentation/glyph-review/arabic-manual-review-dashboard.html` and linked reports. | pending | `make arabic-visual-review-update REVIEW_KEY=smoke-lam-alef REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `class-arabic-farsi-numerals` | Glyph class | arabic-farsi-numerals | Contour decisions pending: 4; fix-now: 0 | U+0660-U+0669 and U+06F0-U+06F9 rhythm, width, and style fit | Section-targeted dashboard snapshot shows Arabic-Indic digit rhythm across all generated fonts. Open the dashboard and glyph sources before judging digit widths and style fit. | Open `documentation/glyph-review/arabic-manual-review-dashboard.html` and linked reports. | pending | `make arabic-visual-review-update REVIEW_KEY=class-arabic-farsi-numerals REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |
| `class-arabic-punctuation` | Glyph class | arabic-punctuation | Contour decisions pending: 4; fix-now: 0 | Arabic comma, semicolon, question mark, per mille, date separator, full stop, and parentheses | Section-targeted dashboard snapshot shows Arabic punctuation across all generated fonts. Review comma, semicolon, question mark, per mille, date separator, full stop, and parentheses in RTL context. | Open `documentation/glyph-review/arabic-manual-review-dashboard.html` and linked reports. | pending | `make arabic-visual-review-update REVIEW_KEY=class-arabic-punctuation REVIEW_STATUS=pass REVIEWER="Name YYYY-MM-DD" NOTES="reviewed"` |

## After Each Batch

```bash
make reports-only
make preflight-only
```

Run `make contour-cleanup-proof` after source edits so the proof, queue, and decision log stay synchronized.
